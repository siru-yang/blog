...
BOM：浏览器对象模型 browser object model，js可以通过bom操作浏览器
...
clientTop(clientLeft)//上方的(左侧)border宽度
scrollTop(scrollLeft)//滚动后上方(左侧)被隐藏的距离
```
...
null和undefined：null是不存在的值，undefined是已声明未赋值。!null !undefined !0都是true
...
1.先判断缓存中是否存在请求数据，不存在则发起请求，并将数据缓存。2.如果存在缓存根据cache-control和expires（cache-control优先级高）判断是缓存是否过期，如果有效，直接使用缓存。3.如果过期向浏览器发起请求，（根据last-modify和eTag字段）携带if-modify-since和if-none-match，浏览器根据这两值判断请求内容是否发生变化，没变化则直接使用缓存，变化则重新返回数据并且更新缓存
...
- [ ] bind实现中，最终实现通过空函数做了中转，为什么可以这么做
- [ ] webpack中 devtool和和devserver的区别
